...
1. **Show the dashboard** — left: requests, middle: agent trace, right: authority
   & audit. The header always shows *Secret visible to model: No* and *Credential
   source: 1Password/runtime*.
2. **SnackHub $42 → Run SpendGuard** → **Allowed**. Trace shows intent extracted,
   no risk flags, policy allowed, **Daytona sandbox created**, credential mounted
   (secret not visible to model), worker executed, audit written.
3. **EventSuppliesCo $76 → Run** → **Allowed**. Same pattern, `create_order`.
4. **Suspicious vendor $900 → Run** → **Blocked** with reasons: prompt injection,
   secret exfiltration, credential forwarding, external destination, unknown
   vendor, over budget. **No Daytona sandbox. No credential. No worker.**
5. **Point at the audit log** — every run is recorded; **no secret anywhere**.
